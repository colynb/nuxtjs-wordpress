 ...
 */
class Custom_Posts_API_Controller extends WP_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function get_item( $request ) {
        ...
        $args = [
            'post_name__in' => [$slug . '__trashed', $slug],
            'post_type' => 'post',
            'post_status' => 'all',
            'posts_per_page'=> 1
        ];
        ...
        if ($slug ==='redirect') {
            return new WP_Error('redirect', ['location' => '/'], ['status' => 301]);
        }
        
        if (!count($posts)) {
            return new WP_Error('not_exists', 'Post Not Found', ['status' => 404]);
        }

        if ($posts[0]->post_status === 'trash') {
            return new WP_Error('deleted', 'Post Deleted', ['status' => 410]);
        }
...
add_action('rest_api_init', function () {
    $controller = new Custom_Posts_API_Controller('post');
